...
<div class={`flex  flex-col ${parseFloat(id) == message.authorID ? ' justify-end' : 'justify-start'}`}>
    {#if message.parentMessage}
        <div class={`flex pt-2 ${parseFloat(id) == message.authorID ? 'justify-end' : "justify-start"}`}>
            <p class="bg-gray-50 rounded-t-2xl px-4 py-2 text-sm inline-block max-w-96 flex-wrap break-all"><span class="italic"> Reply to</span> {message.parentMessage.content}</p>
        </div>
    {/if}
        <div class={`items-center ${parseFloat(id) == message.authorID ? "flex-row-reverse" : ""} flex`}>
